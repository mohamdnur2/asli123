...
client.on('message', ra3d => { // Leaked by [ @!- Jack#0359 ]
            var prefix = "r";
  ...
  client.on('message' , ReBeL => { // Leaked by [ @!- Jack#0359 ]
var prefix = "-";
...
client.on("guildMemberAdd", member => { // Leaked by [ @!- Jack#0359 ]
...
client.on("message",function(message) { // Leaked by [ @!- Jack#0359 ]
...
message.channel.send(`let args = message.content.split(" ").slice(${args}).join(" ")`);
    ...
    if(message.content.startsWith(prefix + "server")) { // Leaked by [ @M!- Jack#0359 ]
...
client.on('guildCreate', guild => { // Leaked by [ @!- Jack#0359 ]
var message = "zg";
         const embed = new Discord.RichEmbed()
     .setColor("GOLD")
...
client.on('guildDelete', guild => { // Leaked by [ @!- Jack#0359 ]
var message = "zg";
         const embed = new Discord.RichEmbed()
     .setColor("GOLD")
     .setTitle('Heart BOT Link Click Here .!')
...
client.on('message', async message => { // Leaked by [ @!- Jack#0359 ]
...
coll.first().delete()
  ...
  e.edit("تم انشاء الغرفه استمتع")
	...
	client.on('message', async msg => { // Leaked by [ @!- Jack#0359 ]
	var prefix = "$";
	var user = msg.author;
...
client.on('message',   eyad =>{ // Leaked by [ @!- Jack#0359 ]
...
// شف دسكورد
client.on('message', msg => { // Leaked by [ @!- Jack#0359 ]
...
client.login("NTEwNzQ3NzI1NTMzNDEzMzc2.DshLnw.d99mrdMd6-Oey7dJoMdv8mlDjX4");
